  ...
  return (
    <Modal visible={visible} animationType="none" transparent statusBarTranslucent>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }}>
        <Animated.View style={[panelStyle, { flex: 1, marginTop: 100, backgroundColor: 'white', borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: insets.bottom }]}>
          ...
          >
            <Pressable onPress={animateClose} style={{ width: 24, alignItems: 'center' }}>
              <Ionicons name="close" size={24} color="black" />
            </Pressable>
            <Text className="flex-1 text-lg font-bold text-black text-center">{title}</Text>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                <View style={{ width: 24, alignItems: 'center', marginRight: 16, marginTop: -1 }}>
                  {isMultiSelect ? (
                    <View
                      className={`w-6 h-6 rounded border-2 items-center justify-center ${
                        isSelected(item.id) ? "bg-pink-600 border-pink-600" : "border-gray bg-white"
                      }`}
                    >
                      {isSelected(item.id) && (
                    ...
                    <View
                      style={isSelected(item.id)
                        ? { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: '#db2777', alignItems: 'center', justifyContent: 'center' }
                        : { width: 20, height: 20, borderRadius: 10, borderWidth: 1, borderColor: '#000000', alignItems: 'center', justifyContent: 'center' }
                      }
                    >
                      {isSelected(item.id) && (
                ...
                <Text
                  ...
                  style={!isSelected(item.id) ? { color: '#000000' } : undefined}
                >
                  {item.val}
                </Text>
              </Pressable>
            ...
            ListEmptyComponent={() => (
              <View className="flex-1 items-center justify-center">
                ...
                <Text className="text-gray-500 text-base mt-2 font-regular">No options found</Text>
              </View>
            )}
          />
          ...
          {isMultiSelect && (
            <View className="bg-white border-t border-gray-200 px-5 py-4 gap-3 pb-6">
              <Pressable onPress={handleReset} className="py-3 bg-gray-100 rounded-full items-center active:bg-gray-200">
                <Text className="text-gray-800 font-semibold text-base">Reset</Text>
              </Pressable>
              <Pressable onPress={handleSave} className="py-3 bg-pink-600 rounded-full items-center active:bg-pink-700">
